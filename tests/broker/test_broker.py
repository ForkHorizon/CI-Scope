...
def test_webhook_rebinds_active_runner_to_actual_github_job(monkeypatch):
    ...
    monkeypatch.setattr(
        broker,
        "webhook_job_payload",
        lambda _payload, _registry, _profiles: (actual, {"id": "profile"}, [], None),
    )
    ...
    actives = saved[-1][0]
    assert actives[0]["id"] == "actual-job"
    assert actives[0]["pid"] == 4242
    assert actives[0]["runnerName"] == "ci-scope-2002-12345"
