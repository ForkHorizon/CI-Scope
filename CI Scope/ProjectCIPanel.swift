...
struct ProjectCIPanel: View {
    ...
    @State private var codeLinterStatus: CodeLinterWorkflowStatus = .unavailable
    ...
    @ViewBuilder
    private func codeLinterStatusRow(script: AutomationScript, workflow: GitHubWorkflow) -> some View {
        HStack(spacing: 8) {
    ...
    private var codeLinterScript: AutomationScript? {
        scripts.first(where: \.isCodeLinter)
    }

    private var codeLinterWorkflowPath: String? {
        codeLinterScript?.matchingWorkflow(in: snapshot)?.path
    }

    private func refreshCodeLinterStatus() async {
        guard let script = codeLinterScript,
              let path = codeLinterWorkflowPath
        else {
