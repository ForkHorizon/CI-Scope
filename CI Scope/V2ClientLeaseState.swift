...
nonisolated enum V2ClientControlLeaseState: Codable, Equatable, Sendable {
    case inactive
    ...
    func canClaim(at now: Date) -> Bool {
        guard case .active(_, let expiresAt) = self else { return false }
        return expiresAt > now
    }
}

nonisolated enum V2ClientControlLeaseEvent: Sendable {
