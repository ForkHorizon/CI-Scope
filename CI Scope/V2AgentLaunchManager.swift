...
struct V2AgentLaunchManager {
    ...
    func installOrUpdateLaunchAgent() async throws -> String {
        ...
        let controlPlaneURL =
            settings.serverURL.isEmpty ? "https://ci.forkhorizon.com" : settings.serverURL
        let machineID =
            settings.machineID.isEmpty ? (Host.current().localizedName ?? "macbook") : settings.machineID
        ...
        if changed {
            _ = await ShellClient.run(
                "launchctl bootout gui/\(uid)/\(Self.serviceLabel) >/dev/null 2>&1 || true", timeout: 8,
                config: config)
        }
        ...
        _ = await ShellClient.run(
            "launchctl kickstart -k gui/\(uid)/\(Self.serviceLabel) >/dev/null 2>&1 || true", timeout: 8,
            config: config)
    ...
    func stopLegacyBroker() async {
        let uid = geteuid()
        _ = await ShellClient.run(
            "launchctl bootout gui/\(uid)/\(Self.legacyServiceLabel) >/dev/null 2>&1 || true", timeout: 5,
            config: config)
    }
